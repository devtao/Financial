...
import me.tspace.authority.pojo.AuthorityConstant;
...
public class ModuleController {
	...
	public String module(Model model,HttpSession session) {
		...
		sb.append("select a.module_id   as i, ").append("       a.module_name as n, ").append("       a.en_module_name as e ").append("from   base_modules a ")
		...
		for (int i = 0; i < list.size(); i++) {
			...
			map.put(temp.get("i"), AuthorityConstant.USER_LOCATION_PRC.equals(user.getLocation())?temp.get("n"):temp.get("e"));
		}
		map.put("0", AuthorityConstant.USER_LOCATION_PRC.equals(user.getLocation())?"主菜单":"Main Menu");
	...
	public void all(PrintWriter writer,HttpSession session,Locale locale) throws IOException {
		BaseUsers user = (BaseUsers) session
				.getAttribute(WebConstants.CURRENT_USER);
		Criteria criteria = new Criteria();
		Tree tree = this.baseModulesService.selectAllModules(criteria,user,messageSource,locale);
