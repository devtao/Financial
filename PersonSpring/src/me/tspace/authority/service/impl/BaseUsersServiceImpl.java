...
import me.tspace.authority.dao.BaseUsersMapper;
...
import me.tspace.common.utils.DataUtil;
...
public class BaseUsersServiceImpl implements BaseUsersService {
	...
	@Override
	public String selectByBaseUser(MessageSource messageSource,Criteria criteria,Locale locale) {
		// 条件查询
		...
		if (dataBaseUser.getErrorCount() >= 3 && compareTo(dataBaseUser.getLastLoginTime())) {
			return returnMsg(messageSource,"user.error7",locale);
		}
		...
		if (!passwordIn.equals(dataBaseUser.getPassword())) {
			// 密码不正确
			return loginTimes(messageSource,dataBaseUser,criteria,locale);
			//return "00";
		}
		// 更新最后登录时间和登录ip
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED, rollbackFor = { Exception.class })
	public String updateUserPassword(MessageSource messageSource,Criteria example,Locale locale) {
		...
		if (!userId.equals(user.getUserId()) || !oldPassword.equals(user.getPassword())) {
			return returnMsg(messageSource,"user.error8",locale);
		}
		// 保存新密码
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED, rollbackFor = { Exception.class })
	public String saveUser(MessageSource messageSource,Criteria example,Locale locale) {
		BaseUsers user = (BaseUsers) example.get("user");
		...
		if (exit) {
			return returnMsg(messageSource,"user.error9",locale);
		}
		int result = 0;
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED, rollbackFor = { Exception.class })
	public String resetPwdByPrimaryKey(MessageSource messageSource,Criteria example,Locale locale) {
		String userId = example.getAsString("userId");
		BaseUsers oldUser = this.baseUsersMapper.selectByPrimaryKey(userId);
		if (oldUser == null) {
			return returnMsg(messageSource,"user.error10",locale);
		}
	 ...
	 * @return
	 */
	private String loginTimes(MessageSource messageSource,BaseUsers baseUsers, Criteria criteria,Locale locale) {
		String info = "";
		...
		case 0:
			// 第一次输入密码错误
			info = returnMsg(messageSource,"user.error11",locale);
			errorCount = (short) (errorCount + 1);
			break;
		case 1:
			// 第二次输入密码错误
			info = returnMsg(messageSource,"user.error12",locale);
			errorCount = (short) (errorCount + 1);
			break;
		case 2:
			// 第三次输入密码错误
			info = returnMsg(messageSource,"user.error13",locale);
			errorCount = (short) (errorCount + 1);
			break;
		default:
			break;
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED, rollbackFor = { Exception.class })
	public String findPassword(MessageSource messageSource,Criteria example,Locale locale) throws Exception {
		BaseUsers user = (BaseUsers) example.get("user");
		//查询是否存在
		...
		if (null == list || list.size() != 1) {
			return returnMsg(messageSource,"user.error14",locale);
		}
		BaseUsers dataBaseUser = list.get(0);
		//设置token
		...
		String title=returnFormatMsg(messageSource,"user.error15",locale,new String[]{dataBaseUser.getAccount()});
		String url=systemUrl+token.toUpperCase()+"/"+dataBaseUser.getUserId();
		url="<a href='"+url+"'/>"+url+"</a>";
		//一小时有效
		String body=returnFormatMsg(messageSource,"user.error16",locale,new String[]{url});
	...
	private boolean execSend(String address, String title, String body) throws Exception {
        ...
        props.setProperty("mail.smtp.timeout", "250000");
	...
	private String returnMsg(MessageSource messageSource , String str,Locale locale){
		return messageSource.getMessage(str, null, null, locale);
	}
	private String returnFormatMsg(MessageSource messageSource , String templateName ,Locale locale, String ...str){
		String template = messageSource.getMessage(templateName, null, null, locale);
		return DataUtil.formatStr(template, str);
	}
